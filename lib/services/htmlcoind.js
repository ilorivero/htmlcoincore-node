...
var HTMLCOINRPC = require('htmlcoind-rpc');
...
/**
 * Provides a friendly event driven API to htmlcoind in Node.js. Manages starting and
 * stopping htmlcoind as a child process for application support, as well as connecting
 * to multiple htmlcoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function HTMLCOIN(options) {
  if (!(this instanceof HTMLCOIN)) {
    return new HTMLCOIN(options);
  }
...
util.inherits(HTMLCOIN, Service);

HTMLCOIN.dependencies = [];

HTMLCOIN.DEFAULT_MAX_TXIDS = 1000;
HTMLCOIN.DEFAULT_MAX_HISTORY = 50;
HTMLCOIN.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
HTMLCOIN.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
HTMLCOIN.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
HTMLCOIN.DEFAULT_SPAWN_RESTART_TIME = 5000;
HTMLCOIN.DEFAULT_SPAWN_STOP_TIME = 10000;
HTMLCOIN.DEFAULT_TRY_ALL_INTERVAL = 1000;
HTMLCOIN.DEFAULT_REINDEX_INTERVAL = 10000;
HTMLCOIN.DEFAULT_START_RETRY_INTERVAL = 5000;
HTMLCOIN.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
HTMLCOIN.DEFAULT_TRANSACTION_CONCURRENCY = 5;
HTMLCOIN.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
HTMLCOIN.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || HTMLCOIN.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || HTMLCOIN.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || HTMLCOIN.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || HTMLCOIN.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || HTMLCOIN.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || HTMLCOIN.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || HTMLCOIN.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || HTMLCOIN.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || HTMLCOIN.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || HTMLCOIN.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

HTMLCOIN.prototype._initCaches = function() {
...
HTMLCOIN.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
HTMLCOIN.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
HTMLCOIN.prototype.getPublishEvents = function() {
  return [
    {
...
HTMLCOIN.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'htmlcoind/' + name, 'total:', this.subscriptions[name].length);
};

HTMLCOIN.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'htmlcoind/' + name, 'total:', this.subscriptions[name].length);
};


HTMLCOIN.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
HTMLCOIN.prototype.unsubscribeBalance = function(emitter, addresses) {
...
HTMLCOIN.prototype.unsubscribeBalanceAll = function(emitter) {
...
HTMLCOIN.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'htmlcoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

HTMLCOIN.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'htmlcoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
HTMLCOIN.prototype.unsubscribeAddressAll = function(emitter) {
...
HTMLCOIN.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = HTMLCOIN.DEFAULT_CONFIG_SETTINGS;
...
HTMLCOIN.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
HTMLCOIN.prototype._expandRelativeDatadir = function() {
...
HTMLCOIN.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in htmlcoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in htmlcoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in htmlcoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './htmlcoin.conf');
  if (spawnOptions.configdir != null) {
    configPath = path.resolve(spawnOptions.configdir, './htmlcoin.conf');
  }

  log.info('Using htmlcoin config file:', configPath);
...
HTMLCOIN.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of htmlcoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of htmlcoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of htmlcoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to htmlcoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from htmlcoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from htmlcoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that htmlcoind is undergoing a reindex. ' +
             ...
             'of htmlcoincore-node services will start.');
    node._reindex = true;
  }
};

HTMLCOIN.prototype._resetCaches = function() {
...
HTMLCOIN.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
HTMLCOIN.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

HTMLCOIN.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('HTMLCOIN Daemon Ready');
          callback();
        });
      });
...
HTMLCOIN.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 14889
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 14889;
  }
  return networkOptions;
};

HTMLCOIN.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/htmlcoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/htmlcoin.conf';
    }
  }
  return networkPath;
};

HTMLCOIN.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
HTMLCOIN.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('htmlcoind/hashblock', message.toString('hex'));
    }
  }

};

HTMLCOIN.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
HTMLCOIN.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('HTMLCOIN Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
HTMLCOIN.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
HTMLCOIN.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
HTMLCOIN.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
HTMLCOIN.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('htmlcoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
HTMLCOIN.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('htmlcoind/rawtransaction', message.toString('hex'));
    }
...
HTMLCOIN.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || HTMLCOIN.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

HTMLCOIN.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
HTMLCOIN.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
HTMLCOIN.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('HTMLCOIN Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || HTMLCOIN.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

HTMLCOIN.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
HTMLCOIN.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/htmlcoind.pid';
...
HTMLCOIN.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting htmlcoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('HTMLCOIN process unexpectedly exited with code:', code);
        log.warn('Restarting htmlcoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('HTMLCOIN process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn htmlcoind.'));
      }
...
HTMLCOIN.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to htmlcoind.'));
    }
 ...
 */
HTMLCOIN.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('HTMLCOIN configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.syncPercentage = function(callback) {
  var self = this;
...
HTMLCOIN.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, htmlcoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
HTMLCOIN.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
HTMLCOIN.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
HTMLCOIN.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
HTMLCOIN.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in htmlcoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
HTMLCOIN.prototype._getConfirmationsDetail = function(transaction) {
...
HTMLCOIN.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
HTMLCOIN.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
HTMLCOIN.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A htmlcoin transaction id
 * @param {Function} callback
 */
HTMLCOIN.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
HTMLCOIN.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
HTMLCOIN.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
HTMLCOIN.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
HTMLCOIN.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
  if (_.isFunction(options) && _.isUndefined(callback)) {
    callback = options;
  }

  this.client.sendRawTransaction(tx, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a HTMLCOINcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
HTMLCOIN.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getInfo = function(callback) {

  var info = {};
  var self = this;

  return async.waterfall([function (callback) {
      return self.client.getNetworkInfo(function(err, response) {
          ...
          info.timeOffset = result.timeoffset;
          info.connections = result.connections;
          info.paytxfee = result.relayfee;
          info.relayFee = result.relayfee;
          info.network = self.node.getNetworkName()
          callback(null, info);
      })
  }, function (info, callback) {
      return self.client.getWalletInfo(function(err, response) {
          if (err) {
              return callback(err);
          }
          var result = response.result;
          info.walletversion = result.walletversion;
          info.balance = result.balance;
          info.keypoololdest = result.keypoololdest;
          info.keypoolsize = result.keypoolsize;
          callback(null, info);
      })
  }, function (info, callback) {
      return self.client.getBlockchainInfo(function(err, response) {
          if (err) {
              return callback(err);
          }
          var result = response.result;
...
HTMLCOIN.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('htmlcoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('htmlcoind process did not exit'));
      }
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
HTMLCOIN.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
HTMLCOIN.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
HTMLCOIN.prototype.getStakingInfo = function(callback) {
    var self = this,
